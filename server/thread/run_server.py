...
def main():
    ...
    parser.add_argument('--debug', action='store_true',
                       help='Enable all debugging options for process pool server')
    ...
    # Add debug options for process pool
    if args.server_type.lower() == 'process' and args.debug:
        cmd.extend(['--debug-connections', '--debug-socket', '--debug-broken-pipe'])
    
    # Show what we're doing
    print(f"Starting {args.server_type}pool server with {args.workers} workers on port {args.port}")
    if args.debug and args.server_type.lower() == 'process':
